...
### Issue #103: スリープ対策（2026-07-29、GAS設定・動作確認完了）

- ブランチ`docs/issue-103-sleep-prevention`で作業。このIssueはRailsアプリ本体のコード変更を伴わず、Google Apps Script（GAS）側の設定が中心のため、リポジトリにはドキュメントのみ記録する方針とした
- Google Apps Script（プロジェクト名`robocup-supporter`）を新規作成した。
- 初回実行して権限承認・エラーなく完了することを確認（実行数画面で`ステータス: 完了`、`期間: 0.729秒`を確認）
- GASの「トリガー」画面から時間主導型・分ベースのタイマー・10分おきのトリガーを登録済み
- 動作確認結果（トリガー登録後、実行数画面で確認）:

  | 開始時間 | 種類 | 期間 |
  | --- | --- | --- |
  | 17:55:26 | エディタ（手動実行） | 0.729秒 |
  | 18:07:37 | 時間主導型 | 1.19秒 |
  | 18:17:37 | 時間主導型 | 1.165秒 |
  | 18:27:37 | 時間主導型 | 1.028秒 |

  - トリガーがほぼ10分間隔で継続的に動作していることを確認。いずれの実行も1秒前後で完了しており、コールドスタート（スリープ復帰）を疑わせる数十秒規模の遅延は見られなかった
  - トリガー稼働開始からおよそ1時間半後、実際にブラウザから本番URLへ手動アクセスし、10秒未満で即座に表示されることを確認
  - さらにその後、時間を空けて再度本番環境へ手動アクセスし、引き続きスリープしておらず即座に表示されることを確認（複数回のスポットチェックでトリガーの有効性を再確認）
- 完了条件の充足状況:
  - 「GASのトリガーが有効化され、10分間隔で本番URLへのアクセスが継続していること」→ 上記実行ログで確認済み
  - 「一定時間アクセスが無い状態からアプリへアクセスしても、スリープによる起動遅延が発生しないこと」→ 手動アクセスで即時表示を確認。ただし、トリガー自体が常時10分間隔でアクセスし続ける設計のため、そもそも「長時間アクセスが無い状態」を意図的に作れない点に留意（トリガーが機能している限りスリープしない、という設計通りの結果）
- 今後の運用: Renderのスリープ仕様が変更される可能性があるため、対策後も定期的に実行数画面・本番URLの表示速度を確認する
- Issueクローズは、上記の確認結果をもって`gh issue close 103`で対応予定（ユーザー承認後に実行）

### Issue #101: 本番環境でのロール別（member/staff）権限動作確認（2026-07-29、確認完了）

- ブランチ`docs/issue-101-role-verification`で作業。ユーザー登録画面ではroleを指定できない仕様（`user_params`が意図的に`role`を許可していない、マスアサインメント対策）のため、本番環境での確認手順を整備した上で対応した
- 本番環境（https://robocup-supporter.onrender.com）の新規登録画面から、確認用のデモアカウントを2件作成
  - `member-demo@example.com`（作成直後はmemberロール）
  - `staff-demo@example.com`（作成直後はmemberロール）
- Neon SQL Editor（`production`ブランチ、`neondb`）で以下のSQLを実行し、`staff-demo@example.com`のroleをstaffへ更新
  ```sql
  UPDATE users SET role = 1 WHERE email = 'staff-demo@example.com';
  ```
  - 実行前後で`SELECT email, role FROM users WHERE email IN (...)`を実行し、`member-demo@example.com`はrole=0（member）のまま、`staff-demo@example.com`はrole=0→1（staff）に更新されたことを確認
- 本番環境で各デモアカウントにログインし、ロールごとの権限動作を確認
  - `member-demo@example.com`: スケジュール・教材の新規登録/編集/削除ボタンが表示されないこと、該当画面への直接アクセス時にリダイレクトされることを確認
  - `staff-demo@example.com`: スケジュールの新規登録・編集・削除、教材の新規登録・削除が問題なく行えることを確認
- 完了条件の充足状況:
  - 「本番環境でmemberロールの場合、スケジュール・教材の新規登録/編集/削除画面・ボタンにアクセスできないこと」→ 確認済み
